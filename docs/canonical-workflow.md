...
All supported formats become a typed `Workflow` with `nodes`, `edges`, variables, source metadata, and preserved provider-specific configuration. The core representation is intentionally engine-neutral so BPMN, generic JSON, n8n, and future UiPath/Temporal/Airflow/LangGraph/Make/Zapier parsers can feed the same validators, evaluators, execution engine, costing, repair, CLI, and UI.
