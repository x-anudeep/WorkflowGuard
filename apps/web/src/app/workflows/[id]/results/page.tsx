...
export default async function WorkflowResultsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <section className="px-5 py-7 lg:px-8">
      <Link href={`/workflows/${workflow.id}`} className="text-sm text-slate-600 hover:text-accent">Back to workflow view</Link>
      <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <h1 className="text-2xl font-semibold">{workflow.name}</h1>
          <p className="mt-1 text-sm text-slate-600">
            Scores, tests, and traces · {formatSourceFormat(workflow.source_format)} · {formatDate(workflow.created_at)}
          </p>
        </div>
